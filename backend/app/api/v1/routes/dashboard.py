...
from app.schemas.dashboard import DashboardResponse, DashboardV2Response
...
@router.get("/dashboard/v2", response_model=DashboardV2Response, tags=["dashboard"])
async def get_dashboard_v2_endpoint(
    ...
    resolved_mes = mes if mes is not None else today.month
    resolved_year = year if year is not None else today.year
    return dashboard_service.get_dashboard_v2(
        user_jwt=token,
        user_id=current_user["user_id"],
        mes=resolved_mes,
        year=resolved_year,
